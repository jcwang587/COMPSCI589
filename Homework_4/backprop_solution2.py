...
def nn_gradient(nn_params, input_layer_size, hidden_layer_size, num_labels, X, y, lambd):
    ...
    print(y)
    print(h)
    J1 = - np.dot(np.transpose(y), np.log(h.reshape(-1, 1))) - np.dot(np.transpose(1 - y), np.log(1 - h.reshape(-1, 1)))
    print(J1)
    ...
    print('Expected output for instance 1: [%6.5f  %6.5f]' % (y[0, 0], y[0, 1]))
    ...
    for i in range(m):
        ...
        delta4[i, :] = h[i, :] - y[i, :]
        delta3[i, :] = np.dot(delta3[i, :].reshape(1, -1), Theta2_x) * sigmoid_gradient(z2[i, :])
