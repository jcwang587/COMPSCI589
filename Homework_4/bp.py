...
def train(network):
    ...
    network.train(labels, data_set, 0.3, 50000)
...
def correct_ratio(network):
    ...
    correct = 0.0
    for i in range(256):
        if normalizer.denorm(network.predict(list(normalizer.norm(i)))) == i:
            correct += 1.0
    print('correct_ratio: %.2f%%' % (correct / 256 * 100))


def netword_error(vec1, vec2):
    aa = 0.5 * reduce(lambda a, b: a + b,
...
def gradient_check(network, sample_feature, sample_label):
    ...
    for conn in network.connections.connections:
        ...
        epsilon = 0.0001
        conn.weight += epsilon
        error1 = netword_error(network.predict(sample_feature), sample_label)

        # 减去一个很小的值，计算网络的误差
        conn.weight -= 2 * epsilon  # 刚才加过了一次，因此这里需要减去2倍
        error2 = netword_error(network.predict(sample_feature), sample_label)
